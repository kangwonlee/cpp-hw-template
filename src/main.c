...
// 이 파일을 수정하여 제출하시오.
// Do not add extra includes or change function signatures—this may break grading.

// Include necessary header for input/output
#include <stdio.h>


// Function prototypes: Declare the functions with their return types and parameters
...
int main() {
    ...
    // Use scanf("%d %d", &a, &b); here (or scanf_s on Windows)
    // 여기에 scanf("%d %d", &a, &b); 사용 (Windows에서는 scanf_s)

    // Prompt for and read two floats
    // 두 실수를 입력받기 위한 프롬프트와 읽기
    printf("Enter two floats (c d): ");
    // Use scanf("%f %f", &c, &d); here (or scanf_s on Windows)
    // 여기에 scanf("%f %f", &c, &d); 사용 (Windows에서는 scanf_s)

    // Print the first separator
    // 첫 번째 구분자 출력
    printf("##########\n");

    // Print integer values and results using the functions
    ...
    // Print float values and results using the functions (with %.2f for formatting)
